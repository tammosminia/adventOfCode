...
val input = """toggle 461,550 through 564,900
              ...
              |turn on 222,12 through 856,241""".stripMargin.lines.toList

type Coordinate = (Int, Int)
class Lights(width: Int, height: Int) {
  var lights: Map[Coordinate, Boolean] = Map().withDefaultValue(false)
  ...
  def newValue(oldValue: Boolean, action: String) = action match {
    case "turn on" => true
    case "turn off" => false
    case "toggle" => !oldValue
  }

  def listCoordinates(upperLeft: Coordinate, downRight: Coordinate) = for {
    x <- upperLeft._1 to downRight._1
    y <- upperLeft._2 to downRight._2
  } yield (x, y)

  def amountOn = listCoordinates((0, 0), (width - 1, height -1)).count { c =>
    lights(c)
  }
}

class VariableLights(width: Int, height: Int) {
  ...
  def newValue(oldValue: Int, action: String) = action match {
    case "turn on" => oldValue + 1
    case "turn off" => List(0, oldValue - 1).max
    case "toggle" => oldValue + 2
  }
  ...
  def amountOn = listCoordinates((0, 0), (width - 1, height -1)).map { c =>
    lights(c)
  }.sum
}

val l = new Lights(1000, 1000)
//l.change("turn on 0,0 through 999,999")
//l.change("toggle 0,0 through 999,0")
//l.change("turn off 499,499 through 500,500")
input.foreach(l.change)
l.amountOn

val v = new VariableLights(1000, 1000)
//l.change("turn on 0,0 through 999,999")
//l.change("toggle 0,0 through 999,0")
//l.change("turn off 499,499 through 500,500")
input.foreach(v.change)
v.amountOn
